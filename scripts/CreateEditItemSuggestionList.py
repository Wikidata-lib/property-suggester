...
if __name__ == "__main__":
    ...
    parser.add_argument("offset", nargs='?', default="0", help="offset")
    parser.add_argument("limit", nargs='?', default="1000", help="limit")
    ...
    for title, claims in CsvReader.read_csv(args.input):
        ...
        if itemCount%100 == 0:
            print str(itemCount)
        if itemCount < int(args.offset):
            continue
        if itemCount > int(args.limit):
            break
        if(len(claims) > 1):
            ...
            for suggestion in suggestions:
                if(float(suggestion["correlation"]) > 0.3):
                    if args.properties == "" or (str(suggestion["id"][1:]) in args.properties.split(",")):
